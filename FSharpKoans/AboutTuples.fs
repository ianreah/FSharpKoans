...
module ``about tuples`` =
    ...
    (* THINK ABOUT IT: Is there really more than one return value?
       ...
       It returns a tuple. The type is shown with an asterisk
       separating the types of the items, i.e., float * float
       It's then using pattern matching to unpack the tuple into
       the two values
    *)
